...
Public API:
- `Future<void> start()` — run the signal-based auto-detect round-trip on Start (note 01/08: open each rear sensor in probe-order with torch + a short coverage dwell, lock the first covered; or honour a pinned `useCamera`). On a lock, keep that `CameraController` streaming via `startImageStream` → a `StreamController<CameraImage>` bridge → `FlutterPPGService.processImageStream`. **Spike-confirmed camera config (note 03):** `ResolutionPreset.low` (→ 320×240 on the A70), audio off, `imageFormatGroup` = `bgra8888` on iOS / `yuv420` on Android, `setFlashMode(FlashMode.torch)` (holds the torch through capture — no native torch code needed), best-effort `setExposureMode(locked)` (works on the A70) and `setFocusMode(locked)` (best-effort — times out on some devices, catch and continue). On no covered sensor, surface a typed `CameraPpgError` (note 06) and return to idle.
- `Future<void> stop()` / `Future<void> dispose()` — release through the single ordered `_release()` (note 17): stop image stream → **close the `StreamController<CameraImage>` bridge before cancelling the `PPGSignal` subscription** (flutter_ppg's `processImageStream` is an `async*` generator on `await for` — cancelling first deadlocks, froze the A70) → dispose the service → torch off → dispose the controller.
...
- Camera selection is the session's own signal-based auto-detect (note 01/08), run inside `start()`: the session enumerates/probes rear sensors and locks on the covered one, or honours a pinned `useCamera`. **No native code** — enumeration via `availableCameras()`, torch via `setFlashMode(FlashMode.torch)` (the spike confirmed both; the old native-fallback phase is dropped). The only native concern that remains is optional torch-brightness control, deferred behind the roadmap STOP.
