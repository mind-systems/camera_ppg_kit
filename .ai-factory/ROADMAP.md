...
- [x] **Example-panel teardown fix + interaction logging** — first on-device run (Samsung A70) froze after a camera read covered, torch stuck on. Root cause: camera teardown awaited `StreamSubscription.cancel()` while `flutter_ppg`'s `async*` `processImageStream` was still parked on `await for` over an open input — a deadlock. Fixed by closing the input `StreamController` **before** cancelling the subscription in both teardown paths (`coverage_detector.dart`, `measurement_runner.dart`); also fixed a `RenderFlex` overflow in the inspector's elapsed row. Established the example-app logging convention (every button via `ppgTap`, coarse lifecycle milestones, one `ppgLog`/`ppgTap` helper) and recorded it in CLAUDE.md. Confirms both Phase-2 panels run a full open→detect→teardown→inspect cycle end-to-end on real hardware (A70: 3 physical rear sensors collapse to 1 logical back via CameraX, as note 03 predicted).
