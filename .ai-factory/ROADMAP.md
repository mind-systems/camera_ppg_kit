...
- [ ] **Calibration RR-capture + Android file export** — a plain-Dart recorder in `example/` (observing `CameraPpgService`'s streams, not owning a session) that accumulates every `RrInterval` (elapsed-ms, rrMs, isArtifact, latest SQI) plus run metadata — effective gate/policy params, camera id, duration, and a summary (total/accepted/artifact counts, mean accepted RR, derived kit BPM) — and writes one self-describing JSON file per run to the app external files dir (`path_provider`'s `getExternalStorageDirectory()` → `adb pull`-able without root). Data layer only, no UI; adds `path_provider`. Spec: `.ai-factory/notes/20-calibration-capture-export.md`.
