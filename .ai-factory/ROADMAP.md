...
- [ ] **Calibration screen (3rd example tab)** — a stripped, quiet tab driving the same `CameraPpgService` (auto-detect, kit defaults): Start, a large elapsed timer shown against a fixed one-minute target for the tester to count their own pulse against, live BPM/SQI/state, a screen-owned wall-clock **auto-stop at 60 s** (not the kit's measuring-time `done`, so the count window and capture window are the same wall-clock interval), an optional counted-beats input, and a "Save run" that writes the note-20 file and surfaces its path on-screen + in the log. A manual Stop shares the same finalize path. Extends the tab shell to three tabs and generalizes the camera-release-on-leave rule to fire when leaving any service-owning tab. Spec: `.ai-factory/notes/21-calibration-screen.md`.
